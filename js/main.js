...
const logoOpen = document.querySelector('.header__logo');
if (iconMenu) { //проверяем есть ли такой класс
	iconMenu.addEventListener("click", function (e) { //вешаем на иконку событие клик
		...
		logoOpen.classList.toggle('_opened');
		function logo() {
			if (!iconMenu.classList.contains('_active')) {
				logoOpen.classList.remove('_opened');
			}
		}
		logo();
	});
...
if (profileAvatar) { //проверяем есть ли такой класс
	profileAvatar.addEventListener("click", function (e) { //вешаем на иконку событие клик
		profileMenu.classList.toggle('_active');//ставим и убираем класс _active на иконку бургера каждый раз при клике (для навешивания анимаций при клике на иконку бургера)
...
$(document).on('click touchstart', function (e) {
	if (!$(e.target).is(".profile__avatar *")) {
		$('.profile__menu').removeClass('_active');
	};
});
...
const dateItems = document.querySelectorAll('.item-session');

for (let dateItem of dateItems) {
	//dateItem.addEventListener('click', (e) => e.preventDefault());

	dateItem.addEventListener('click', (event) => {
		for (let i = 0; i < dateItems.length; i++) {
			dateItems[i].classList.remove('_selected');
		}
		dateItem.classList.add('_selected');
	});
}
...
schemeSvg.addEventListener('click', (event) => {
	if (!event.target.classList.contains('booked')) {
		...
		let totalSeats = schemeSvg.querySelectorAll('.selected').length;
